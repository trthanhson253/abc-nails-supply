...
const handleClose = () => {
  ...
  const handleRemove = async (slug,cateId,images) => {
    ...
    if (window.confirm("ATTENTION! When this category is deleted, all of its sub-categories and products will be deleted also. The data will be deleted permanently and cannot be recovered .Are you sure you want to continue this process ?")) {
    //   setLoading(true);
  ...
  return (
   ...
   <AdminMenu />
   <div id="page-wrapper">
   <CategoryCreateModal open={open} handleClose={handleClose} loadCategories={loadCategories}
    removeImage={removeImage} token={token} setRemoveImage={setRemoveImage}/>
                       ...
                       <table className="table table-striped table-bordered table-hover" id="dataTables-example">
                           ...
                           {categories.map((c) => (
                            <tr className="odd gradeX" key={c._id}>
                                ...
                                <button type="button" class="btn btn-danger btn-circle" onClick={() => handleRemove(c.slug,c._id,c.images)}><i class="fa fa-trash-o"></i></button></div>                            
                                </td>
                            </tr>
                          ))}          
                           </tbody>
